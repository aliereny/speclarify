        ...
        (set, get) => ({
            ...
            fetchProject: async (projectId) => {
                ...
                try {
                    ...
                    set({ projects: get().projects.map(p => p.id === projectId ? response.data : p) });
                } catch (error) {
            ...
            addProject: async (name, description) => {
                ...
                try {
                    ...
                    const newProject: Project = { ...response.data, created_at: '', number_of_requirements: 0 }; // Placeholder values for created_at and number_of_requirements
            ...
            updateProject: async (projectId, name, description) => {
                set({ loading: true, error: null });
                try {
                    await axiosClient.put(`/projects/${projectId}`, { name, description });
                    const updatedProject: Project = { id: projectId, name, description, created_at: '', number_of_requirements: 0 }; // Placeholder values for created_at and number_of_requirements
