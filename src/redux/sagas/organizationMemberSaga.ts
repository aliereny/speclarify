...
export function* fetchOrganizationMembersSaga(action: PayloadAction<FetchOrganizationMembersRequestPayload>) {
  try {
    const response: AxiosResponse<ApiResponse<PageResponse<OrganizationMember>>> = yield call(jwtAxios.get, `/organizations/${action.payload.orgPath}/members`, {
...
export function* fetchOrganizationMemberSaga(action: PayloadAction<FetchOrganizationMemberRequestPayload>) {
  try {
    const response: AxiosResponse<ApiResponse<OrganizationMember>> = yield call(jwtAxios.get, `/organizations/${action.payload.orgPath}/members/${action.payload.memberId}`);
...
export function* createOrganizationMemberSaga(action: PayloadAction<CreateOrganizationMemberRequestPayload>) {
  ...
  try {
    yield call(jwtAxios.post, `/organizations/${orgPath}/members`, rest);
...
export function* updateOrganizationMemberSaga(action: PayloadAction<UpdateOrganizationMemberRequestPayload>) {
  ...
  try {
    yield call(jwtAxios.put, `/organizations/${orgPath}/members/${memberId}`, rest);
...
export function* deleteOrganizationMemberSaga(action: PayloadAction<DeleteOrganizationMemberRequestPayload>) {
  try {
    yield call(jwtAxios.delete, `/organizations/${action.payload.orgPath}/members/${action.payload.memberId}`);
