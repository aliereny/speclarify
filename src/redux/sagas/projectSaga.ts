...
export function* fetchProjectsSaga(action: PayloadAction<FetchProjectsRequestPayload>) {
  try {
    const response: AxiosResponse<ApiResponse<PageResponse<Project>>> = yield call(ApiClient.get, `/organizations/${action.payload.orgPath}/projects`, {
...
export function* fetchProjectSaga(action: PayloadAction<FetchProjectRequestPayload>) {
  try {
    const response: AxiosResponse<ApiResponse<Project>> = yield call(ApiClient.get, `/organizations/${action.payload.orgPath}/projects/${action.payload.projectPath}`);
...
export function* createProjectSaga(action: PayloadAction<CreateProjectRequestPayload>) {
  try {
    ...
    yield call(ApiClient.post, `/organizations/${action.payload.orgPath}/projects`, formData, {
      headers: {
...
export function* updateProjectSaga(action: PayloadAction<UpdateProjectRequestPayload>) {
  try {
    ...
    yield call(ApiClient.put, `/organizations/${action.payload.orgPath}/projects/${action.payload.projectPath}`, formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
    });
...
export function* deleteProjectSaga(action: PayloadAction<DeleteProjectRequestPayload>) {
  try {
    yield call(ApiClient.delete, `/organizations/${action.payload.orgPath}/projects/${action.payload.projectPath}`);
